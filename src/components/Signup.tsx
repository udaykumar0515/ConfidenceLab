...
function Signup({ onSignup, onSwitchToLogin }: SignupProps) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-50 via-slate-50 to-emerald-50">
      <div className="absolute inset-0 overflow-hidden">
        ...
        <div className="absolute animate-pulse delay-100 top-40 right-20 w-64 h-64 bg-emerald-200 rounded-full mix-blend-multiply filter blur-xl opacity-70"></div>
        <div className="absolute animate-pulse delay-200 bottom-20 left-1/2 w-64 h-64 bg-violet-200 rounded-full mix-blend-multiply filter blur-xl opacity-70"></div>
      </div>
      ...
      <div className="bg-white/80 backdrop-blur-lg rounded-xl shadow-xl p-8 w-full max-w-md relative transform hover:scale-105 transition-transform duration-300">
        <div className="flex flex-col items-center mb-8">
          <div className="relative mb-4">
            <img src="/logo.png" alt="ConfidenceLab Logo" className="w-16 h-16 object-contain" />
            <Sparkles className="absolute -right-2 -top-2 w-6 h-6 text-emerald-500 animate-spin" />
          </div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">ConfidenceLab</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full flex justify-center py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-gradient-to-r from-blue-600 to-emerald-600 hover:from-blue-700 hover:to-emerald-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transform transition-all duration-200 hover:scale-105 hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
          >
